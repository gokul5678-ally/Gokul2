...
Features:
- Fixed Real Gmail opening: Direct on-demand IMAP UID message fetching (no lost caches).
- HTML email sanitization for safe Telegram rendering without ParseMode errors.
...
# ============================================================================
# ROBUST REAL GMAIL IMAP CLIENT (ON-DEMAND & UID-BASED)
# ============================================================================
def _decode_header_str(raw_header: Optional[str]) -> str:
    if not raw_header:
        return ""
    try:
...
def _extract_plain_text(raw_email: email.message.Message) -> str:
    body = ""
        ...
        for p in raw_email.walk():
            if p.get_content_type() == "text/plain":
                ...
                if payload:
                    ...
                    break
        # Fallback preference: strip text from text/html
        if not body:
            for p in raw_email.walk():
...
def fetch_gmail_inbox_headers_sync(user_email: str, app_password: str) -> List[Dict[str, Any]]:
    """Fetches high-level header metadata for all inbox messages using IMAP UIDs."""
    ...
    try:
        ...
        for u in reversed(uids):
            uid_str = u.decode()
            res, data = mail.uid("fetch", u, "(BODY.PEEK[HEADER.FIELDS (SUBJECT FROM DATE)])")
            if res == "OK" and data and isinstance(data[0], tuple):
                header_msg = email.message_from_bytes(data[0][1])
                subj = _decode_header_str(header_msg.get("Subject")) or "(No Subject)"
                sender = _decode_header_str(header_msg.get("From")) or "Unknown Sender"
                results.append({
                    "id": uid_str,
        ...
        logger.warning(f"Gmail IMAP header fetch error for {user_email}: {e}")
        return []

    return results


def fetch_gmail_single_message_sync(user_email: str, app_password: str, uid_str: str) -> Optional[Dict[str, Any]]:
    """Directly fetches the full body and details of a single email via IMAP UID on demand."""
    clean_pass = app_password.replace(" ", "")
    try:
        mail = imaplib.IMAP4_SSL("imap.gmail.com", port=993)
        mail.login(user_email, clean_pass)
        ...
        res, data = mail.uid("fetch", uid_str.encode(), "(RFC822)")
        mail.logout()
        ...
        if res == "OK" and data:
            for response_part in data:
                if isinstance(response_part, tuple):
                    ...
                    subj = _decode_header_str(raw_email.get("Subject")) or "(No Subject)"
                    sender = _decode_header_str(raw_email.get("From")) or "Unknown Sender"
        ...
        logger.warning(f"Error retrieving Gmail message UID {uid_str}: {e}")
    return None


async def get_gmail_inbox_headers(user_email: str, app_password: str) -> List[Dict[str, Any]]:
    return await asyncio.to_thread(fetch_gmail_inbox_headers_sync, user_email, app_password)


async def get_gmail_single_message(user_email: str, app_password: str, uid_str: str) -> Optional[Dict[str, Any]]:
    return await asyncio.to_thread(fetch_gmail_single_message_sync, user_email, app_password, uid_str)
...
def build_user_home_screen(user_id: int) -> Tuple[str, InlineKeyboardMarkup]:
    ...
    text = (
        ...
        f"👤 <b>System ID:</b> <code>{html.escape(user_info['system_id'])}</code>\n"
...
def build_mailbox_view(user_id: int, mail_id: str, messages: list, page: int = 0) -> Tuple[str, InlineKeyboardMarkup]:
    ...
    text = (
        f"<b>📧 Mailbox Monitor:</b> <code>{html.escape(address)}</code>\n"
        f"🏷️ <b>Type:</b> {type_badge}\n"
        f"🔑 <b>Pass / App Key:</b> <code>{html.escape(password)}</code>\n"
    ...
    for offset, msg in enumerate(page_messages, start=1):
        ...
        if len(sender_clean) > 24:
            sender_clean = sender_clean[:21] + "..."
        keyboard.append([InlineKeyboardButton(f"💬 #{global_num} From: {sender_clean}", callback_data=f"u:msg:{mail_id}:{msg['id']}")])
            ...
            try:
                ...
                for user_id, active_mail_id in active_users:
                    ...
                    if m_type == "disposable":
                        ...
                        messages = await get_gmail_inbox_headers(mail_data["address"], mail_data["password"])
                    ...
                    if new_messages:
                        for nm in new_messages:
                            ...
                            if m_type == "disposable":
                                ...
                                full_gmail = await get_gmail_single_message(mail_data["address"], mail_data["password"], msg_id)
                                body = full_gmail.get("text", "") if full_gmail else ""

                            otp = extract_security_code(body)

                            alert_text = (
                                f"🚨 <b>New Email Received!</b>\n"
                                f"📬 <b>To:</b> <code>{html.escape(mail_data['address'])}</code>\n"
                                f"👤 <b>From:</b> <code>{html.escape(sender_addr)}</code>\n"
                                f"📝 <b>Subject:</b> {html.escape(subject)}\n"
                            )
                            if otp:
                                alert_text += f"🔑 <b>Detected Code / OTP:</b> <code>{html.escape(otp)}</code>\n"
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        if data == "u:home":
            ...
            if m_type == "disposable" and mdata.get("token"):
                ...
                messages = await get_gmail_inbox_headers(mdata["address"], mdata["password"])
        ...
        # FIXED ON-DEMAND GMAIL OPENING
            ...
            if m_type == "disposable":
                ...
                # Live On-Demand Gmail IMAP fetch by UID
                detail = await get_gmail_single_message(mdata["address"], mdata["password"], msg_id)
                if detail:
                    sender_name = detail.get("from", {}).get("name", "N/A")
                    sender_addr = detail.get("from", {}).get("address", "N/A")
                    subject = detail.get("subject", "(No Subject)")
                    ...
                    await query.answer("Failed to load message from Gmail server.", show_alert=True)
                    return

            otp = extract_security_code(body_text)
            otp_banner = f"\n🔑 <b>Detected Verification Code:</b> <code>{html.escape(otp)}</code>\n" if otp else ""
            
            preview = body_text[:1200] + ("..." if len(body_text) > 1200 else "")
            
            view_text = (
                f"📨 <b>Email Message</b>\n\n"
                f"👤 <b>From:</b> {html.escape(sender_name)} (<code>{html.escape(sender_addr)}</code>)\n"
                f"📝 <b>Subject:</b> {html.escape(subject)}\n"
                f"{otp_banner}\n"
            ...
            try:
                ...
                # Safe plain text fallback if any unescaped tags trigger Telegram parse errors
                fallback_plain = (
                    f"📨 Email Message\n\n"
                    f"From: {sender_name} ({sender_addr})\n"
                    f"Subject: {subject}\n"
                    f"{f'OTP/Code: {otp}' if otp else ''}\n\n"
                    f"Content:\n{preview}"
                )
                await query.edit_message_text(fallback_plain, reply_markup=kb)
...
async def user_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if current_stage == "AWAITING_CUSTOM_EMAIL":
        ...
        await update.message.reply_text(
            f"📧 Address: <code>{html.escape(text)}</code>\n\n"
        ...
        test_fetch = await get_gmail_inbox_headers(email_addr, raw_code)
        ...
        ack = (
            ...
            f"📧 <code>{html.escape(email_addr)}</code>\n"
...
def build_admin_channel_menu() -> Tuple[str, InlineKeyboardMarkup]:
    ...
    if channels:
        ...
        for idx, ch in enumerate(channels, start=1):
            text += f"{idx}. <code>{html.escape(ch)}</code>\n"
    else:
...
async def admin_callback_dispatcher(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "a:dash":
        ...
        text = (
            ...
            f"👤 <b>System ID:</b> <code>{html.escape(uinfo.get('system_id', 'N/A'))}</code>\n"
        ...
        for u in users:
            lines.append(f"• <code>{html.escape(u['system_id'])}</code> | ID: <code>{u['telegram_id']}</code> | Cr: <b>{u['credits']}</b>")
...
async def inspect_network_host(target) -> None:
    ...
    text = f"🌐 <b>Host Network Inspection</b>\n\nPublic Outbound IP: <code>{html.escape(ip)}</code>"
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Dashboard", callback_data="a:dash")]])
...
async def admin_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if stage == "ADD_CHANNEL":
        ...
        if channel_name in channels:
            ...
            await save_storage()
            await update.message.reply_text(
                f"✅ <b>Channel Added:</b> <code>{html.escape(channel_name)}</code>\n"
        ...
        await update.message.reply_text(f"Target: <code>{html.escape(udata['system_id'])}</code>. Send credit amount to grant:")
        return
        ...
        try:
            ...
            await update.message.reply_text(f"🎁 Granted <b>+{cr} Credits</b> to <code>{html.escape(uinfo['system_id'])}</code>!", parse_mode=ParseMode.HTML)
            user_bot: Application = context.application.bot_data["user_bot"]
            try:
                await user_bot.bot.send_message(
                    chat_id=int(uid),
