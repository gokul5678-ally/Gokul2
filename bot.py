...
# ============================================================================
# REAL GMAIL IMAP CLIENT (SYNCHRONOUS WORKER RUN IN THREAD)
# ============================================================================
def fetch_gmail_inbox_sync(user_email: str, app_password: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        recent_ids = msg_ids[-10:]  # Inspect the 10 most recent emails
            ...
            try:
                ...
                for user_id, active_mail_id in active_users:
                    ...
                    if m_type == "disposable":
                        ...
                        # Fetch from Real Gmail using IMAP
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        if data == "u:home":
            ...
            if m_type == "disposable":
                ...
                # Real Gmail message lookup from cache
...
async def user_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if current_stage == "AWAITING_CUSTOM_EMAIL":
        ...
        await update.message.reply_text("⏳ <i>Testing Gmail IMAP connection...</i>", parse_mode=ParseMode.HTML)
        
        # Test real IMAP authentication
        ...
        ack = (
            ...
            f"Live IMAP monitoring is now active!"
        )
