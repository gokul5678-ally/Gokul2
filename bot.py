#!/usr/bin/env python3
"""
Dual-Bot Architecture with Dynamic Multi-Channel Force-Join:
    ...
    if os.path.exists(STORAGE_FILE):
        try:
            with open(STORAGE_FILE, "r", encoding="utf-8") as f:
                ...
                # Ensure migration from old single string to list format
...
async def get_unjoined_channels(bot, user_id: int) -> List[str]:
    """Returns a list of required channels the user has NOT joined yet."""
...
async def mail_tm_create_account(client: httpx.AsyncClient, email: str, password: str) -> Optional[dict]:
    try:
        resp = await client.post(f"{MAIL_TM_BASE}/accounts", json={"address": email, "password": password})
...
async def mail_tm_get_token(client: httpx.AsyncClient, email: str, password: str) -> Optional[str]:
    try:
        resp = await client.post(f"{MAIL_TM_BASE}/token", json={"address": email, "password": password})
        if resp.status_code == 200:
...
def build_user_home_screen(user_id: int) -> Tuple[str, InlineKeyboardMarkup]:
    ...
    keyboard = [
        [
            ...
            InlineKeyboardButton("🔐 Add App Pass Mail (10 Cr)", callback_data="u:add_custom"),
        ],
        [
...
def build_mailbox_view(user_id: int, mail_id: str, messages: list) -> Tuple[str, InlineKeyboardMarkup]:
    user_info = get_or_create_user(user_id)
    ...
    type_badge = "⚡ Disposable" if m_type == "disposable" else "🔐 Custom App Pass"
    text = (
        f"<b>📧 Mailbox Monitor:</b> <code>{address}</code>\n"
        f"🏷️ <b>Type:</b> {type_badge}\n"
        f"🔑 <b>Pass / App Key:</b> <code>{password}</code>\n"
        f"📨 <b>Messages Stored:</b> <code>{len(messages)}</code>\n\n"
    ...
    for idx, msg in enumerate(messages, start=1):
        ...
        if len(sender) > 24:
            sender = sender[:21] + "..."
        keyboard.append([InlineKeyboardButton(f"💬 #{idx} From: {sender}", callback_data=f"u:msg:{mail_id}:{msg['id']}")])
                ...
                for user_id, active_mail_id in active_users:
                    ...
                    if new_messages:
                        for nm in new_messages:
                            ...
                            detail = await mail_tm_fetch_message_detail(client, msg_id, token)
                        ...
                        if user_id in user_active_view:
                            ...
                            v_text, v_kb = build_mailbox_view(user_id, active_mail_id, messages)
                            try:
...
async def user_start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # If completely verified and bonus unclaimed, award 50 bonus credits
...
async def user_callback_dispatcher(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        # Open mailbox
            ...
            mdata = boxes[mail_id]
            messages = []
            if mdata.get("type") == "disposable" and mdata.get("token"):
                messages = await mail_tm_fetch_inbox(client, mdata["token"])

            text, kb = build_mailbox_view(user_id, mail_id, messages)
            await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=kb)
        ...
        # Read message
            ...
            if not mdata or not mdata.get("token"):
            ...
            if not detail:
                await query.answer("Unable to load email content.", show_alert=True)
                return
            ...
            kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Mailbox", callback_data=f"u:open:{mail_id}")]])
        ...
        # Disposable account generation
            ...
            created = await mail_tm_create_account(client, email, password)
            if not created:
            ...
            token = await mail_tm_get_token(client, email, password)
            if not token:
            ...
            user_active_mailbox[user_id] = mail_id
            text, kb = build_mailbox_view(user_id, mail_id, [])
            await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=kb)
        ...
        # Custom Mail account
            ...
            prompt = (
                f"🔐 <b>Add Mail Account (16-Digit App Password)</b>\n\n"
                f"• Cost: <b>{COST_CUSTOM_MAIL} Credits</b>\n"
                f"• Deletion Policy: <b>Full {REFUND_CUSTOM_MAIL} Credits refunded upon deletion</b>\n\n"
                f"Please reply with your email address:\n"
                f"<i>Example: myaccount@gmail.com</i>"
            )
...
async def user_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if current_stage == "AWAITING_CUSTOM_EMAIL":
        ...
        await update.message.reply_text(
            ...
            f"Now send the <b>16-digit App Password</b>:\n"
        ...
        ack = (
            f"✅ <b>Mailbox Saved Successfully!</b>\n"
...
async def admin_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if stage == "ADD_CHANNEL":
        clean_ch = text.split("/")[-1]  # Support full URL or handle
